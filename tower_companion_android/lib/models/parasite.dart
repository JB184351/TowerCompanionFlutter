class Parasite {
  ...
  Parasite(this.id, this.towerRunId, this.name, this.positiveDescription,
  ...
  static List<Parasite> getAllParasites() {
    ...
    Parasite silphicBladeSpinner =
        Parasite(null, null, "Silphic Bladespinner", bladeSpinner, silphic);

    Parasite silphicGoldSpinner =
        Parasite(null, null, "Silphic Goldspinner", goldSpinner, silphic);

    String barbed = "Weapon Damage decreased by 15%.";
    String fleshStinger = "Damage dealt to weak points increased by 30%.";

    Parasite barbedFleshStinger =
        Parasite(null, null, "Barbed FleshStinger", fleshStinger, barbed);

    parasites.add(barbedFleshStinger);
    parasites.add(silphicGoldSpinner);
    parasites.add(silphicBladeSpinner);
    ...
    for (var negativePassivePrefix in negativePassivePrefixes.entries) {
      for (var positivePassiveSuffix in positivePassiveSuffixes.entries) {
        ...
        Parasite parasite = Parasite(null, null, parasiteName,
            positivePassiveSuffix.value, negativePassivePrefix.value);
    ...
    parasites.sort((a, b) => a.name.compareTo(b.name));
