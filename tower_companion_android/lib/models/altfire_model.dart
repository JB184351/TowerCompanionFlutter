class AltFire_Model {
  ...
  AltFire_Model(this.name, this.level, this.altFireDescription);

  static List<AltFire_Model> getAllAltFires() {
    List<AltFire_Model> altfires = [];

    final blastShell = AltFire_Model("Blast Shell", 3,
        "Lobs a grenade-light projectile that explodes on contact with an enemy, or after enough time has passed.");
    final doomBringer = AltFire_Model("Doom Bringer", 3,
    ...
    final tendrilpod = AltFire_Model("Tendrilpod", 3,
  ...
  String getAltFireDescription(String altfireName) {
    ...
    final altFire = allAltfires.firstWhere(
        (altFire) => altFire.name == altfireName,
        orElse: () => allAltfires[0]);
    return altFire.altFireDescription;
  }
}
